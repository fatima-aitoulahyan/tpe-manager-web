...
export default function FactureEditPage() {
    ...
    const { fetchOne, update, loading: saving } = useFactures();
    ...
    const loadClients = async (selectNewClientId = null) => {
        try {
            ...
            toast.error(t('facture_edit_toast_error_clients', "Erreur lors du chargement des clients"));
        } finally {
    ...
    useEffect(() => {
        const initData = async () => {
            try {
                const [factureData] = await Promise.all([
                    fetchOne(id),
                    loadClients()
                ]);

                setClientId(
                    typeof factureData.client === 'object'
                        ? String(factureData.client.id)
                        : String(factureData.client)
                );
                setDateEcheance(factureData.date_echeance ?? '');
                setTva(factureData.taux_tva ?? 20);
                setModePaiement(factureData.mode_paiement ?? 'VIREMENT');
                setConditions(factureData.conditions ?? '');
                setLignes(factureData.lignes ?? []);
            } catch (err) {
                toast.error(t('facture_edit_toast_error_load', "Erreur lors du chargement de la facture"));
                navigate('/factures');
            } finally {
                setLoading(false);
            }
    ...
    const handleQuickClientSubmit = async (clientFormData) => {
        ...
        try {
            ...
            toast.success(t('facture_edit_toast_client_success', "Client {{nom}} créé avec succès !").replace('{{nom}}', newClient.nom));
            setIsClientModalOpen(false);
            await loadClients(newClient.id);
        } catch (err) {
            toast.error(err.message || t('facture_edit_toast_client_error', "Échec de la création du client"));
        } finally {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!clientId) return toast.error(t('facture_edit_error_client', "Sélectionnez un client"));
        if (!dateEcheance) return toast.error(t('facture_edit_error_date', "Date d'échéance requise"));
        if (lignes.length === 0) return toast.error(t('facture_edit_error_lignes', "Ajoutez au moins une ligne"));
        ...
        try {
            ...
            toast.success(t('facture_edit_toast_success', "Facture mise à jour avec succès !"));
            navigate(`/factures/${id}`);
        } catch (err) {
            toast.error(err.message || t('facture_edit_toast_error_update', "Erreur lors de la mise à jour"));
        }
    };

    if (loading) return (
        <div className="flex items-center justify-center h-64 text-slate-400 text-sm">
            {t('facture_edit_loading', "Chargement de la facture...")}
        </div>
    );
        ...
        <>
            <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl mx-auto p-1 relative">
                ...
                <div className="flex items-center gap-3 pt-10 sm:pt-0">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900">{t('facture_edit_title', "Modifier la facture")}</h1>
                        <p className="text-slate-500 text-sm mt-0.5">{t('facture_edit_subtitle', "Modifiez les informations de la facture.")}</p>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            {loadingClients ? (
                                <div className="text-xs text-slate-400 mt-6">{t('facture_clients_loading', "Chargement...")}</div>
                            ) : (
                                <ClientSelector
                                    clients={clients}
                        ...
                        <div>
                            <label className="block text-xs font-medium text-slate-700 mb-1">
                                {t('facture_label_echeance', "Date d'échéance")}
                            </label>
                            <input
                                type="date"
                                required
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-xs font-medium text-slate-700 mb-1">
                                {t('facture_label_paiement', "Mode de règlement")}
                            </label>
                            <select
                                value={modePaiement}
                    ...
                    <div>
                        <label className="block text-xs font-medium text-slate-700 mb-2">
                            {t('facture_label_lignes', "Lignes de facturation")}
                        </label>
                    ...
                    {lignes.length > 0 && (
                        <div className="border border-slate-100 rounded-xl overflow-hidden mt-3 text-sm">
                            {lignes.map((l, index) => (
                                <div
                                    key={index}
                                    className="flex items-center justify-between p-3 bg-slate-50/50 border-b border-slate-100"
                                >
                                    <div>
                    ...
                    <div>
                        <label className="block text-xs font-medium text-slate-700 mb-1">
                            {t('facture_label_conditions', "Conditions particulières (Optionnel)")}
                        </label>
                        <textarea
                            value={conditions}
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl space-y-2 text-sm">
                        <div className="flex justify-between text-slate-500">
                            <span>{t('facture_total_ht', "Total HT")}</span>
                    ...
                    >
                        {saving ? t('facture_edit_saving', "Enregistrement...") : t('facture_edit_submit_btn', "Enregistrer les modifications")}
                    </button>
                </div>
            </form>
